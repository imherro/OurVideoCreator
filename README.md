...
管理员在 `/admin` 的四张快捷卡片填写 RunningHub、幻场 AI、火山引擎和豆包语音 Key，保存时原子补齐并发布单人版默认模型；无需逐个填写 Provider 和模型 JSON。RunningHub 一个 Key 同时配置文本与媒体两个显式地址。默认取自单人版公开代码 `564dd22`，不复制其账号配置；完整对应关系和验证边界见 [四服务快捷配置](docs/provider-setup.md)。其他服务及自定义参数保留在折叠的高级配置中。

“保持原 Key”“替换 Key”“吊销旧版本”是不同操作。已配置卡片的 Key 留空表示保留，填写则替换；不会回显原 Key。勾选“同时设为对应用途的默认模型”时，同用途以最后一次保存为默认，不改已有作品模型选择。正常轮换保留旧任务的配置/凭证版本，已有远端任务继续原账号/地址查询；吊销会阻止旧凭证的查询/取消，不尝试用新账号补发任务。停用停止新提交和未出站任务，仍允许未吊销凭证查询已有远端任务。缺版本引用的旧任务没有 settings 明文回落。
