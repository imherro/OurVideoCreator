...
test('independent Y update survives X receipt and is never echoed by the host',async()=>{
  const f=fixture();f.page.edit(doc=>{doc.shots[0].action='v2';});
  const pending=f.page.save();await f.committed;
  await f.other(doc=>{doc.shots[1].action='remote y2';});
  f.page.remote([copy(f.server.get('shot-y'))]);f.release();await pending;await f.page.save();
  assert.equal(f.page.state.doc.shots[1].action,'remote y2');
  assert.equal(f.first.baseline.parts.get('shot:y').content.shot.action,'remote y2');
  assert.equal(f.requests.filter(item=>item.label==='first').length,1);
  assert.equal(f.page.state.conflict,false);
});

test('an unrelated object can still be saved independently while X is conflicted',async()=>{
  const f=await race();
  f.page.edit(doc=>{doc.shots[1].note='independent note';});await f.page.saveObject('shot-y');
  assert.equal(f.server.get('shot-y').content.shot.note,'independent note');
  assert.equal(f.first.drafts.entries.get('shot-x').state,'conflict');
  assert.equal(f.server.get('shot-x').revision,3);
  assert.equal(f.page.state.conflict,true);
});

test('epoch change before delayed receipt cannot be reused for a stale draft',async()=>{
  const f=fixture();f.page.edit(doc=>{doc.shots[0].action='v2';});
  const pending=f.page.save();await f.committed;
  // Test-only management event: assignment increments revision AND epoch.
  const reassigned={...copy(f.server.get('shot-x')),revision:3,assignment_epoch:2,assignee_id:'editor-b'};
  f.server.set('shot-x',copy(reassigned));f.page.remote([reassigned]);f.release();await pending;
  const draft=f.first.drafts.entries.get('shot-x');
  assert.equal(draft.state,'conflict');assert.equal(draft.base.assignment_epoch,1);
  assert.equal(draft.remote.assignment_epoch,2);
  assert.throws(()=>f.first.resolve('shot-x',reassigned,'keep-draft',f.page.state.doc,[]),/负责人/);
  f.page.acceptObjectDocument(f.first.resolve('shot-x',reassigned,'discard',f.page.state.doc,[]));
  f.page.edit(doc=>{doc.shots[0].note='former owner';});
  await assert.rejects(f.page.saveObject('shot-x'),/负责人/);
  assert.equal(f.requests.length,1);assert.equal(f.server.get('shot-x').revision,3);
});

for(const only of [false,true])for(const sameId of [false,true])test(`old scope receipt cannot affect new host (only=${only}, sameId=${sameId})`,async()=>{
  const f=fixture();f.page.edit(doc=>{doc.shots[0].action='old scope v2';});
  const pending=(only?f.page.saveObject('shot-x'):f.page.save()).catch(error=>error);
  await f.committed;
  const next=copy(f.snapshot());next.id=sameId?'episode':'new-episode';next.document.shots[0].action='new scope content';
  next.objects.find(row=>row.id==='shot-x').content.shot.action='new scope content';
  f.first.open(next,[]);f.page.refs.current.current={project:next,doc:next.document};
  f.page.acceptObjectDocument(next.document);
  const before=copy(f.page.state);f.release();await pending;
  assert.deepEqual(f.page.state,before);
  assert.equal(f.page.refs.dirty.current,false);
  assert.equal(f.first.rows.get('shot:x').content.shot.action,'new scope content');
});
