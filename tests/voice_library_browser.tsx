...
const noop=()=>{};
function App(){const [doc,setDoc]=useState(initial),[focus,setFocus]=useState('state-v'),[changes,setChanges]=useState(0);
 const update=(fn:any)=>{setDoc(fn);setChanges(n=>n+1)};
 const common:any={visual:doc.filmBible.visual,shots:[],assets:[],jobs:[],voiceProfiles:doc.filmBible.voices.profiles,
 providers:[{id:'speech',name:'隔离语音模型',kind:'audio',type:'volcengine_speech',rules:{voice_type:{enum:['voice']}},defaults:{voice_type:'voice'}}],
 ...
 onRenameCard:noop,onDeleteCard:noop,onSaveVersion:noop,onStatus:noop,onSetImageOverride:noop,onUploadReference:noop,onGenerateReference:noop,onLock:noop,onFork:noop,onUpgrade:noop,onBind:noop,onUnbind:noop,onLocate:noop,onPreviewAsset:noop};
