...
由 Codex 按独立小批次复现、实现、验证、提交并推送 origin 同名分支，无需 ChatGPT 外审。保持五角色 R1–R6、PostgreSQL、平台独占 Provider/Key、服务端权限、对象版本/租约、显式采纳和审核历史。复用 7878 验收数据及单 Worker，历史付费验证已结束，不是新调用授权。

用户随后明确“AI助手先不做”：上下文助手、聊天、流程建议及其导航/指南增量暂缓。智能文档导入仍在后续队列。以后若恢复助手开发，原约束仍是无费用确认/配额平台、每次模型调用持久记录；本轮不实施助手。
