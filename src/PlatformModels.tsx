    ...
    <div className="platform-model-grid">
      <div>
        ...
        <form onSubmit={e => {e.preventDefault(); void act(async () => {
          ...
          <label>协议类型<select value={provider.config.type} onChange={e => setProvider({...provider, config: {...provider.config, type: e.target.value}})}>
          ...
          <label>认证方式<select value={provider.config.auth_mode || 'api_key'} onChange={e => {setReplaceKey(true); setKey(''); setProvider({...provider, config: {...provider.config, auth_mode: e.target.value}});}}>
            <option value="api_key">API Key</option><option value="none">无认证（仅明确支持的私有服务）</option>
          </select></label>
      ...
      <div>
        ...
        <form onSubmit={e => {e.preventDefault(); void act(async () => {
          ...
          <p className="muted">仅允许规则中列出的参数；支持 integer/number/string/boolean/strings 及 min/max/enum/max_length。</p>
