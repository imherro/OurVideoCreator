...
export function StoryboardWorkspace(props: Props) {
  ...
  return <section className="storyboard-workspace">{header}<div className="storyboard-table">{shots.map((shot, index) => {
    ...
    const shotFields = <div className="storyboard-shot-fields">
    ...
    const bindingEditor = <div className="storyboard-bindings"><div><b>VisualVersion 绑定</b><small>直接引用 Production Film Bible</small></div><div className="storyboard-reference-list">{references.map((item) => {
    ...
    return <article key={uid} className={selected.includes(uid) ? "selected" : ""}>
      ...
      <div className="storyboard-table-main">
        ...
        {props.purpose === "planning" ? shotFields : <div className="storyboard-image-summary">
          <h3>{shot.scene || "未命名场景"}</h3>
          <p className="storyboard-action-summary">{shot.action || "尚未填写动作"}</p>
          <div className="storyboard-image-references">{references.map(referenceChip)}</div>
          {!references.length && <small className="storyboard-reference-warning">尚未绑定角色、场景或道具</small>}
      ...
      {props.purpose === "planning" ? <>{bindingEditor}{promptEditor}</> : <details className="storyboard-image-details">
        ...
        {bindingEditor}
        {node && props.renderImageSettings?.(node)}
        {promptEditor}
      </details>}
