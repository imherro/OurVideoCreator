...
export function voiceSampleRows(document:Value,shot:Value,assets:Value[]){
 ...
 return (shot.dialogues||[]).filter((line:Value)=>{
  ...
  let profile:Value={},error='';try{profile=resolvedVoice(document,shot,line).profile}catch(reason:any){error=reason.message}
  const asset=assets.find(item=>item.id===profile.referenceAssetId&&item.kind==='audio');
  return {cardId:line.characterCardId,name:line.characterName||'角色',profile,asset,
   error,ready:!error&&profile.status==='locked'&&!!asset&&profile.referenceVersion===profile.version};
 });
}
