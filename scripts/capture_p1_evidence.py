"""Run the P1 acceptance commands and retain their combined, unedited output."""
...
OUT = ROOT / "docs" / "multiuser-rollout" / "evidence" / "P1"
...
COMMANDS = [
    ...
    ("06-process-boundary", [sys.executable, "-m", "pytest", "-q", "tests/test_p1_processes.py", "-s"]),
    ("07-ffmpeg-export", [sys.executable, "-m", "pytest", "-q", "tests/test_export.py", "-s"]),
    (
        "08-zero-upstream-on-missing-provider",
        [
    ...
    ("09-route-audit", [sys.executable, "scripts/audit_routes.py"]),
...
def main() -> int:
    ...
    (OUT / "11-removed-surface.json").write_text(
        json.dumps(surface, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    summary = {
        "evidence_kind": "P1 final acceptance run",
        ...
        "test_isolation": "pytest creates a temporary MVC_DATA_DIR; process test creates another isolated shared data directory",
        "paid_egress_guard": "pytest blocks non-loopback socket connections; fake Provider is loopback-only; no real credentials are supplied",
