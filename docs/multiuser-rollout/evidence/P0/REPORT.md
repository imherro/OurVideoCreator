...
外部验收在 `3c0e5ca` 给出不通过，问题编号为 OVC-P0-01 至 OVC-P0-05。原始 P0 执行当时未保存 stdout/stderr，不能事后伪造；因此 P0-R1 以同一业务 SHA 重新运行，并明确标记为补跑。修正文档、只读脚本、逐项处理结果和原始日志索引见 `../P0-R1/REPORT.md` 与该目录的 `SUMMARY.json`。P1 仍未开始。
