...
| `backend/instance_identity.py` / `Studio-Process.ps1` | 用脚本工程根 + 实际数据目录生成实例 ID；身份查询受控切换到目标根并核对返回根，相对数据目录也按该根绝对化；Start/Stop 在最外层 `finally` 恢复调用者的 `MVC_DATA_DIR` 与 `PYTHONUTF8`；生命周期记录校验角色、PID、创建时间、可执行文件和完整命令行 | P1-R1/P1-R2/P1-R3 补强本机进程所有权与同会话隔离；P7 部署加固 |
