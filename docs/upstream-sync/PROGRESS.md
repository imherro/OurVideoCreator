...
关键结论：初剪同一视频轨已经由协作版现有实现满足，不重复移植；状态资产 waiting、Provider 凭证规范化、LAN HTTP 请求 ID 是优先复现的小修候选；智能文档导入实际跨 6 个提交，应作为连续功能链分步实现；Production 回收与共享视觉风格必须拆开；120 秒默认和“所有模型”策略不照搬。AI 助手会产生外部费用，须另行明确费用/确认策略。
