...
> 2026-09-19 接手更新：当前检查点 `c813043`、旧项状态与新增 16 提交见 [剩余清单](2026-09-19-REMAINING.md)。下方 `13939a8` 是历史冻结点，不是当前上游最新。AI 助手按用户最新指示暂缓；五角色 R1–R6 已交付，详见 `../five-role-rollout.md`。本轮 SYNC-40 记录在文末。
...
## UPSTREAM-SYNC-37：重新获取并审计上游新增 23 提交

用户指出单机版持续更新后，重新获取只读 `upstream/master`，确认旧冻结点 `13939a8` 之后新增 23 个提交，最新为 `564dd22`；范围共 95 文件、3913 行增加、407 行删除。逐提交读取差异、测试和说明文档，并与协作版画布、初剪、批量资产、音色、创建页、作品库、回收、视觉风格、平台模型和凭证路径对照。完整结论见 [新增 23 提交增量清单](2026-09-17-INCREMENT-13939a8-564dd22.md)。

关键结论：初剪同一视频轨已经由协作版现有实现满足，不重复移植；状态资产 waiting、Provider 凭证规范化、LAN HTTP 请求 ID 是优先复现的小修候选；智能文档导入实际跨 6 个提交，应作为连续功能链分步实现；Production 回收与共享视觉风格必须拆开。清单提交时暂缓的 120 秒默认、“所有模型”策略和 AI 助手，随后均由用户明确纳入实施范围，最新决定见 SYNC-38 及增量清单。

本批只改文档，没有产品代码、依赖、迁移、配置或后台页面变化；产品测试、构建、浏览器 E2E、数据库集成和真实 Provider 均未运行。清单提交推送后按用户“一阶段一暂停”的要求停下，下一阶段建议先复现状态资产依赖是否被错误归类为 blocked。

## UPSTREAM-SYNC-38：新作品默认单集 120 秒

用户在增量清单提交后明确要求采用上游 `ab528e0` 的产品默认。协作版网页创建草稿与后端 `new_document` 统一从 15 秒改为 120 秒；名称-only 兼容创建会同时得到 120 秒的项目文档和改编策划目标。显式传入 5–3000 秒仍覆盖默认值，后续 Episode 继续继承同一 Production 上一集的已保存时长。

本批不迁移或改写已有作品；后端对缺字段旧文档的运行时兼容回落仍保持 15 秒，避免把“新建默认”扩大为历史数据迁移。没有新增依赖、数据库迁移、配置或后台页面，也没有 Provider 调用。

验证先把前端默认断言改为 120，得到预期红测 **4 passed / 1 failed（实际 15）**；实现后定向 **5 passed**。完整 `npm test`：**287 passed / 0 failed / 0 skipped，1532.10ms**；`npm run build`：tsc/Vite exit 0，Vite **6.93秒**，保留既有大 chunk 警告。Python 纯函数回归确认 `new_document()` 为 120，已有 `duration:15` 文档经迁移仍保持 15，`compileall` 通过。真实 PostgreSQL API 定向测试已尝试，但当前 shell 缺少必需的 `OVC_TEST_ADMIN_URL`，在测试数据库创建前退出，明确记为**未运行**；没有改用验收库代替测试库。

本阶段进行期间，用户另外明确要求后续实施“新项目包含全部已启用系统模型”和 AI 助手。AI 助手暂不建设费用确认或配额，但每次模型调用必须留下可查询的持久调用记录。这两项只登记为后续已授权范围，不与 120 秒默认值混成同一提交；下一阶段先核对并实现全部启用平台模型语义。

## UPSTREAM-SYNC-40：状态资产等待基础图的批量提示

2026-09-19，来源 `d437dd34758652ed1c45c2e0a394e55d1aa34c03`。此前 SYNC-39 的平台模型范围实施记录位于增量清单第 6 节；本轮不重复开发模型池或五角色成果。接手与新范围详见 [剩余清单](2026-09-19-REMAINING.md)。用户本轮随后明确“AI助手先不做”，该决定覆盖此前助手实施计划。

先在当前协作版实际运行新的业务期望：`node --test tests/batch_generation.test.mjs` 得到 **3 passed / 1 failed**，父图 draft 的状态卡被列入 blocked，错误为“需要先确认并锁定父版本参考图”。并非只根据上游提交标题推断。

最小适配：批量计划新增 waiting 分类；基础图生成中、候选待采纳、待锁定、失败/中断分别给出下一步提示。waiting 不进入 readyIds 或提交数量，也不显示为错误。父版本不存在、弃用、回收、归属不匹配或锁定但无主图仍 blocked；回收的卡片不参与计划。实际按钮与提交后反馈显示等待数量、基础版本和原因，明确要求人工采纳、锁定后再次点击，不自动续跑。服务端拒绝和真正提交错误仍保留。

本批只调整前端计划与反馈，仍调用原 generateVisualReference 服务端接口；未修改服务端对象权限、版本/租约、五角色审核和候选采纳。当前服务端 `validate_visual_reference_job` 已拒绝未锁定父版本和缺主参考图，故不把等待提示作为放宽准入的理由。未复制上游自动同步候选语义。

实际验证：

- 计划/视觉参考定向 **13 passed**，覆盖父任务 queued/running/succeeded/interrupted/failed、人工采纳后仍待锁定、锁定后才可生成、非法父关系、回收、只读规划和错误保留。
- `npx tsc -b` 通过；全量前端 `npm test` **291 passed / 0 failed / 0 skipped**（1534.59ms）。
- `npx vite build --outDir .local/upstream-sync-40-build` 通过（6.95s），仅独立暂存产物，保留原大 chunk 警告。
- 随后增加实际 `runSmartBatch` 函数执行测试，模拟服务端提交，验证只提交就绪父图、全等待零提交、无权限错误不被吞掉；最终本文件 **8 passed**（138.80ms）。包含先前已跑的用例，不与 291 相加宣称全仓 299；新增用例后未重复全量测试。Node stripTypeScriptTypes 的实验性警告保留。
- 清单编号覆盖、16 个新增 SHA 与 Git 范围一致性，以及 `git diff --check` 在提交前核验。

未验证：本轮未运行真实 PostgreSQL 集成、浏览器 E2E、全量后端或真实 Provider；后端无修改，不把历史 PG/真实联调结果当作本轮测试。没有新增依赖、迁移、后台页面或费用确认/配额平台，真实调用 0。7878 现用构建、验收数据、Key 与单 Worker 保持；未部署此批暂存前端，未触碰单机服务。

交付状态 READY_FOR_REVIEW，仅推送 origin 同名集成分支，不需外部审核，不代表全部上游完成。下一小批次优先 LAN HTTP 请求 ID 复现，然后继续已授权智能导入；AI 助手暂缓。
